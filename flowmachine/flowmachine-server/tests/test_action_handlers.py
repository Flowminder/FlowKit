...
import pytest
import flowmachine_server
from flowmachine_server.action_handlers import (
    get_action_handler,
...
from flowmachine_core.core.context import get_redis, get_db, redis_connection
from flowmachine_core.core.init import connections
from flowmachine_core.core.cache import reset_cache, get_query_object_by_id
...
async def test_run_query_type_error(monkeypatch, server_config):
    ...
    monkeypatch.setattr(
        flowmachine_server.action_handlers,
        "get_schema",
        Mock(return_value=BrokenSchema),
    )
    msg = await action_handler__run_query(config=server_config, query_kind={})
    assert msg.status == ZMQReplyStatus.ERROR
    assert (
        msg.msg
        == "Internal flowmachine-queries-server server error: could not create query object using query schema. The original error was: 'DUMMY_FAIL_MESSAGE'"
    )
