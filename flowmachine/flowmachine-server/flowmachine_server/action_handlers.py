...
# supported by the flowmachine-queries-server server. In order to implement a new action
...
from flowmachine_core.core.cache import get_query_object_by_id
from flowmachine_server.query_info_lookup import (
    QueryInfoLookup,
    UnkownQueryIdError,
    QueryInfoLookupError,
)
from flowmachine_core.core.query_state import QueryStateMachine, QueryState
from flowmachine_core.utils import convert_dict_keys_to_strings
from flowmachine_server.exceptions import FlowmachineServerError
from flowmachine_server.query_schemas.flowmachine_query import (
    get_query_schema,
    get_schema,
)
from flowmachine_server.zmq_helpers import ZMQReply
...
async def action_handler__get_available_queries(
    ...
    Returns a list of available flowmachine-queries-server queries.
    """
    available_queries = list(get_schema().type_schemas.keys())
...
async def action_handler__get_query_schemas(
    ...
    Returns a dict with all supported flowmachine-queries-server queries as keys
...
async def action_handler__run_query(
    ...
    Constructs a flowmachine-queries-server query object, sets it running and returns the query_id.
    For this action handler the `action_params` are exactly the query kind plus the
    parameters needed to construct the query.
    """
    try:
        query_obj = get_schema()().load(action_params)
        ...
        error_msg = (
            f"Internal flowmachine-queries-server server error: could not create query object using query schema. "
...
async def action_handler__get_geography(
    ...
    try:
        query_obj = (
            get_schema()
            .type_schemas["geography"]()
            .load({"aggregation_unit": aggregation_unit})
        )
    except TypeError as exc:
        # We need to catch TypeError here, otherwise they propagate up to
        # perform_action() and result in a very misleading error message.
...
async def perform_action(
    ...
    try:
        ...
        error_msg = f"Internal flowmachine-queries-server server error: wrong arguments passed to handler for action '{action_name}'."
    ...
    if not isinstance(reply, ZMQReply):
        error_msg = f"Internal flowmachine-queries-server server error: handler for action '{action_name}' returned an invalid reply."
        raise FlowmachineServerError(error_msg)
