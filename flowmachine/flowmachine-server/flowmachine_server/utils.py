...
async def send_zmq_message_and_await_reply(msg, port=5555, host="localhost"):
    """
    Helper function to send JSON messages to the flowmachine-queries-server server (via zmq) and await a reply.
    ...
    port : int or str
        Port on which the flowmachine-queries-server server is running (default: 5555)

    host : str
        The host on which the flowmachine-queries-server server is running (default: 'localhost')
...
def send_zmq_message_and_receive_reply(msg, port=5555, host="localhost"):
    """
    Helper function to send JSON messages to the flowmachine-queries-server server (via zmq) and receive a reply.
