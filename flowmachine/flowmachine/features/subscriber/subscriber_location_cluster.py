...
def subscriber_location_cluster(
    ...
    **kwargs,
    ...
    """
    Represents a location clustering class. This is a wrapper for all the
    location clustering methods available in `flowmachine`.

    This class allows the subscriber to produce location clusters just based on
    CDR parameters like start and stop dates, hours, etc without the need
    to pass method specific data formats.
    ...
    if subscriber_identifier not in ("msisdn", "imei", "imsi"):
        raise ValueError("Unidentified subscriber id: {}".format(subscriber_identifier))

    if method not in ("hartigan"):
