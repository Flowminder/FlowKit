...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("stddev", "V7MBRewnwQGE91gY", "versioned-cell", 182.519128)],
)
def test_contact_reference_location_stats(get_dataframe, statistic, msisdn, level, want):
    """ Test a few hand-picked ContactReferenceLocationStats. """
    cb = ContactBalance("2016-01-01", "2016-01-03")
    ml = ModalLocation(*[
        daily_location(d, level=level, subscriber_subset=cb.counterparts_subset(include_subscribers=True))
...
def test_contact_reference_location_stats_custom_geometry(get_dataframe):
    ...
    cb = ContactBalance("2016-01-01", "2016-01-03")
    ml = ModalLocation(*[
        daily_location(d, level="versioned-cell", subscriber_subset=cb.counterparts_subset(include_subscribers=True))
        for d in list_of_dates("2016-01-01", "2016-01-03")
    ])
    cb.store()
    ml.store()
    ml = CustomQuery(f"SELECT subscriber, ST_POINT(lon, lat) AS loc FROM ({ml.get_query()}) _", ["subscriber", "loc"])
...
def test_contact_reference_location_stats_errors(kwarg):
    ...
    if kwarg == "contact_locations":
        ml = ModalLocation(*[
            daily_location(d, level="admin3", subscriber_subset=cb.counterparts_subset(include_subscribers=True))
            for d in list_of_dates("2016-01-01", "2016-01-03")
        ])
