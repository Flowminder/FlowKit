...
@pytest.mark.parametrize(
    ...
    [("avg", "gwAynWXp4eWvxGP7", 298.7215), ("stddev", "V7MBRewnwQGE91gY", 182.519128)],
)
def test_contact_modal_location_distance(get_dataframe, statistic, msisdn, want):
    ...
    query = ContactModalLocationDistance(
        "2016-01-01", "2016-01-03", statistic=statistic
    )
...
def test_contact_modal_location_errors(kwarg):
    ...
    error = (
        "error"
        if kwarg not in {"contact_balance"}
