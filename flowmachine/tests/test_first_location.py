...
def test_time_at_first_location_correct(get_dataframe):
    ...
    assert str(set_df.ix["038OVABN11Ak4W5P"]) == "2016-01-01 05:02:10+00:00"
    assert str(set_df.ix["1p4MYbA1Y4bZzBQa"]) == "2016-01-02 21:30:41+00:00"
    assert str(set_df.ix["3XKdxqvyNxO2vLD1"]) == "2016-01-01 05:09:20+00:00"
...
def test_handles_list_of_locations(get_dataframe):
    ...
    assert str(df.ix["038OVABN11Ak4W5P"]) == "2016-01-01 05:02:10+00:00"
...
def test_can_be_called_with_any(get_dataframe):
    ...
    df = get_dataframe(dfl)
    df.set_index("subscriber", inplace=True)
    assert str(df.ix["0MQ4RYeKn7lryxGa"]) == "2016-01-03 01:38:56+00:00"
