...
from flowmachine.features import CallDays
import numpy as np
...
def test_calldays_column_names(exemplar_level_param):
    ...
    cd = CallDays("2016-01-01", "2016-01-03", **exemplar_level_param)
...
def test_call_days_returns_expected_counts_per_subscriber(get_dataframe):
    ...
    for (subscriber, start, end, calls) in test_values:
        cd = CallDays(start, end, level="versioned-site")
...
def test_locations_are_only_repeated_once_per_subscriber(get_dataframe):
    ...
    cd = CallDays("2016-01-01", "2016-01-03", level="cell")
    df = get_dataframe(cd)
