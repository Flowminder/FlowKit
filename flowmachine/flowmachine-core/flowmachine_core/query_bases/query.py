...
from flowmachine_core.core.errors.flowmachine_errors import QueryResetFailedException
from flowmachine_core.core.query_state import QueryStateMachine
from abc import ABCMeta, abstractmethod

from flowmachine_core.core.errors import (
    NameTooLongError,
    NotConnectedError,
    UnstorableQueryError,
)

from flowmachine_core.utils import _sleep
from flowmachine_core.core.dependency_graph import store_all_unstored_dependencies

from flowmachine_core.core.cache import write_query_to_cache
...
class Query(metaclass=ABCMeta):
    """
    The core base class of the flowmachine_core module. This should handle
    ...
    def query_state(self) -> "QueryState":
        ...
        Returns
        -------
        flowmachine_core.core.query_state.QueryState
    ...
    def query_state_str(self) -> str:
        ...
        str
            ...
            defined in `flowmachine_core.core.query_state.QueryState`.
        """
    ...
    def union(self, other, all=True):
        ...
        from flowmachine_core.utility_queries.union import Union
    ...
    def subset(self, col, subset):
        ...
        from flowmachine_core.utility_queries.subset import subset_factory
    ...
    def numeric_subset(self, col, low, high):
        ...
        from flowmachine_core.utility_queries.subset import subset_numbers_factory
    ...
    def index_cols(self):
        ...
        try:
            ...
            pass
        try:
            if "subscriber" in cols:
                ixen.append('"subscriber"')
            elif self.subscriber_identifier in cols:
    ...
    def random_sample(self, sampling_method="random_ids", **params):
        ...
        See Also
        --------
        flowmachine_core.core.random.random_factory

        Notes
        -----
        Random samples may only be stored if a seed is supplied.
        """
        from flowmachine_core.utility_queries.random import random_factory
