...
# -*- coding: utf-8 -*-
"""
This module provides initial setup routines for flowmachine_core. From a user
...
from flowmachine_core.core import Connection
from flowmachine_core.core.context import bind_context, context, get_db
from flowmachine_core.core.errors import NotConnectedError
from flowmachine_core.core.logging import set_log_level
from get_secret_or_env_var import environ, getenv

logger = structlog.get_logger("flowmachine_core.debug", submodule=__name__)
...
def connections(
    *,
    ...
    """
    Context manager which connects flowmachine_core to a database, and performs initial set-up routines.
    ...
    After connecting, you should use `flowmachine_core.core.context.get_db()` to access
    the database connection, `flowmachine_core.core.context.get_redis()` to get the redis
    connection, and `flowmachine_core.core.context.get_executor()` to get the threadpool.
    ...
    flowdb_user : str, default "flowmachine_core"
    ...
    conn : flowmachine_core.core.Connection
...
def connect(
    *,
    ...
    """
    Connects flowmachine_core to a database, and performs initial set-up routines.
    You may provide a Settings object here, which can specify the database
    you wish to connect to, logging behaviour, available tables and so on.
...
def _do_connect(
    *,
    ...
    try:
        ...
        flowdb_user = (
            getenv("FLOWMACHINE_FLOWDB_USER", "flowmachine_core")
    ...
    set_log_level("flowmachine_core.debug", log_level)
    ...
    print(f"FlowMachine version: {flowmachine_core.__version__}")
