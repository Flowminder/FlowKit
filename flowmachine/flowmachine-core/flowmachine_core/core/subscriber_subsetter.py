...
class SubscriberSubsetterForFlowmachineQuery(SubscriberSubsetterBase):
    """
    Represents a subset given by a flowmachine_core query.
    """
    ...
    def __init__(self, flowmachine_query):
        ...
        Parameters
        ----------
        flowmachine_query : flowmachine_core.Query
            The flowmachine_core query to be used for subsetting. The only requirement
    ...
    def _verify_that_subscriber_column_is_present(self, flowmachine_query):
        """
        Check that the flowmachine_core query contains a 'subscriber' column and
    ...
    def apply_subset_if_needed(self, sql, *, subscriber_identifier=None):
        ...
        # Create a sqlalchemy "Textual SQL" object from the flowmachine_core SQL string
        textual_sql = text(self.flowmachine_query.get_query())

        # Create sqlalchemy column objects for each of the columns
        # in the output of the flowmachine_core query
        sqlalchemy_columns = [
...
def make_subscriber_subsetter(subset):
    ...
    Parameters
    ----------
    subset : "all" or None or list or tuple or flowmachine_core.Query or SubscriberSubsetterBase
        This can be one of the following:
          ...
          - flowmachine_core.Query: represents a subset given by the result of a flowmachine_core query
