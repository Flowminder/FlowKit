...
from flowmachine_core.core.context import get_db, get_redis, get_executor
from flowmachine_core.core.query_state import QueryState, QueryStateMachine
...
def test_scoring(test_query, nested_test_query):
    ...
    dl_2 = nested_test_query.store().result()
...
def test_touch_cache_record_for_table():
    """
...
def test_get_cached_query_objects_ordered_by_score(test_query, nested_test_query):
    """
    Test that all records which are queries are returned in correct order.
    """
    dl = test_query.store().result()
    dl_agg = nested_test_query.store().result()
    table = dl.get_table()
...
def test_get_cached_query_objects_protected_period(test_query, nested_test_query):
    """
...
def test_shrink_one(test_query, nested_test_query):
    """
    Test that shrink_one removes a cache record.
    """
    dl = test_query.store().result()
    dl_aggregate = nested_test_query.store().result()
    get_db().engine.execute(
...
def test_shrink_to_size_does_nothing_when_cache_ok(test_query):
    """
...
def test_shrink_to_size_removes_queries(test_query):
    """
...
def test_shrink_to_size_respects_dry_run(test_query, nested_test_query):
    """
    Test that shrink_below_size doesn't remove anything during a dry run.
    """
    dl = test_query.store().result()
    dl2 = nested_test_query.store().result()
...
def test_shrink_to_size_uses_score(test_query, nested_test_query):
    """
...
def test_size_of_cache(test_query, nested_test_query):
    """
...
def test_size_of_table(test_query):
    """
...
def test_get_query_object_by_id(test_query):
    """
...
def test_delete_query_by_id(test_query):
    """
...
def test_delete_query_by_id_does_not_cascade_by_default(test_query, nested_test_query):
    """
...
def test_delete_query_by_id_can_cascade(test_query, nested_test_query):
    """
...
@pytest.fixture
def flowmachine_connect_with_cache_settings_reset():
    """
...
def test_cache_table_exists(test_query):
    """
    Test that cache_table_exists reports accurately.
    """
    assert not cache_table_exists(get_db(), "NONEXISTENT_CACHE_ID")
    assert cache_table_exists(get_db(), test_query.store().result().query_id)


def test_redis_resync(test_query):
    """
    Test that redis states can be resynced to the flowdb cache.
    """
    stored_query = test_query.store().result()
    assert (
        QueryStateMachine(
...
def test_cache_reset(test_query):
    """
...
def test_cache_reset_protects_tables(get_dataframe):
    """
    Resetting the cache should preserve Table entries.
    """
    # Regression test for https://github.com/Flowminder/FlowKit/issues/832
    assert len(get_db().fetch("SELECT * FROM cache.cached")) == 0
    Table("events.calls_20160101")
    assert len(get_db().fetch("SELECT * FROM cache.cached")) == 1
    reset_cache(get_db(), get_redis())
    assert len(get_db().fetch("SELECT * FROM cache.cached")) == 1


def test_redis_resync_runtimeerror(dummy_redis, test_query):
    """
...
def test_cache_metadata_write_error(dummy_redis, test_query, monkeypatch):
    """
    ...
    monkeypatch.setattr("flowmachine_core.core.cache.write_cache_metadata", writer_mock)
...
@pytest.mark.asyncio
async def test_cache_watch_does_shrink(test_query):
    """
...
@pytest.mark.asyncio
async def test_cache_watch_does_timeout(json_log):
    """
