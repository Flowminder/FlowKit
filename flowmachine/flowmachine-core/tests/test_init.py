...
from flowmachine_core.core import connect


@pytest.fixture(autouse=True)
def reset_connect(monkeypatch):
    logging.getLogger("flowmachine_core.debug").handlers = []
...
def test_double_connect_warning():
    ...
    with pytest.warns(UserWarning):
        connect()
    # assert 1 == len(logging.getLogger("flowmachine_core.debug").handlers)
...
def test_bad_log_level_goes_to_error(monkeypatch):
    ...
    connect()
    assert logging.ERROR == logging.getLogger("flowmachine_core.debug").level
...
def test_log_level_set_env(monkeypatch):
    ...
    connect()
    assert logging.INFO == logging.getLogger("flowmachine_core.debug").level
...
def test_log_level_set(monkeypatch):
    ...
    assert logging.CRITICAL == logging.getLogger("flowmachine_core.debug").level
...
def test_param_priority(mocked_connections, monkeypatch):
    ...
    core_set_log_level_mock.assert_called_with(
        "flowmachine_core.debug", "dummy_log_level"
    )
...
def test_env_priority(mocked_connections, monkeypatch):
    ...
    connect()
    core_set_log_level_mock.assert_called_with(
        "flowmachine_core.debug", "DUMMY_ENV_LOG_LEVEL"
    )
    core_init_Connection_mock.assert_called_with(
        port=6969,
...
def test_connect_defaults(mocked_connections, monkeypatch):
    ...
    core_set_log_level_mock.assert_called_with("flowmachine_core.debug", "error")
    core_init_Connection_mock.assert_called_with(
        port=9000,
        user="flowmachine_core",
        password="foo",
        host="localhost",
        database="flowdb",
        pool_size=5,
