...
from flowmachine_core.core.subscriber_subsetter import *
...
def test_raises_error_if_flowmachine_query_does_not_contain_subscriber_column():
    """
    An error is raised when creating a subsetter from a flowmachine_core query that doesn't contain a column named 'subscriber'.
    """
...
def test_subsetting_of_query(get_dataframe):
    ...
    dl_0 = EventTableSubset(start="2016-01-01", stop="2016-01-02")
    dl_1 = EventTableSubset(
        start="2016-01-01", stop="2016-01-02", subscriber_subset=subsetter_1
    )
    dl_2 = EventTableSubset(
        start="2016-01-01", stop="2016-01-02", subscriber_subset=subsetter_2
    )
    dl_3 = EventTableSubset(
        start="2016-01-01", stop="2016-01-02", subscriber_subset=subsetter_3
    )

    assert 1254 == len(get_dataframe(dl_0))
    assert 1254 == len(get_dataframe(dl_1))
    assert 10 == len(get_dataframe(dl_2))
    assert 70 == len(get_dataframe(dl_3))
