...
## Adding a new server to FlowAuth

There are two steps to adding a server to FlowAuth: configuring FlowAPI to accept tokens generated by the FlowAuth instance, and uploading the FlowAPI instance's API spec to FlowAuth.

As part of the deployment process for FlowAPI, you will need FlowAuth's public key. To obtain the public key, log in to FlowAuth as an administrator and click the Public Key menu item. You can then click the download button to download the key as a file ready for use with Docker secrets, or click the copy encoded button for a base64 encoded version of the key suitable for use as an environment variable.

After obtaining the public key, you can deploy FlowAPI. Two configuration items are relevant here - the `PUBLIC_JWT_SIGNING_KEY` and `FLOWAPI_IDENTIFIER` environment variables or secrets. The first, `PUBLIC_JWT_SIGNING_KEY`, should be either the base64-encoded key-string copied from FlowAuth if providing as an environment variable, or the downloaded key file if you are using secrets. `FLOWAPI_IDENTIFIER` should be a _unique_ identifier for the FlowAPI deployment - this will also be the _name_ of the server in the FlowAuth user interface. For more detail on installation, see the [installation documentation](install.md#prodinstall).

Once FlowAPI has started up, you should be able to download the API specification from `https://<flowapi_host>:<flowapi_port>/api/0/spec/openapi.json`. You can then use the spec file to add the server to FlowAuth by navigating to Servers, and clicking the new server button.
