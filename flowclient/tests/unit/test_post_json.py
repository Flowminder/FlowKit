...
def test_post_json_good_statuses(status_code, session_mock, flowclient_connection):
    """response object should be returned for OK status codes.."""
    session_mock.post("DUMMY_ROUTE", status_code=status_code, content="TEST")
    assert (
        b"TEST" == flowclient_connection.post_json(route="DUMMY_ROUTE", data={}).content
    )


def test_post_json_reraises(session_mock, flowclient_connection):
    """post_json should reraise anything raised by requests."""
    session_mock.post(
        "DUMMY_ROUTE", content=RequestError("DUMMY_MESSAGE", request=None)
    )
...
def test_404_raises_error(session_mock, flowclient_connection):
    ...
    session_mock.post("DUMMY_ROUTE", status_code=404)
    with pytest.raises(
...
def test_access_denied_error(denial_status_code, session_mock, flowclient_connection):
    ...
    session_mock.post(
        ...
        content=ZMQReply(status="error", msg="ERROR_MESSAGE"),
    )
...
def test_access_denied_unknown_error(
    denial_status_code, session_mock, flowclient_connection
):
    """If a msg field is not available for an access denied a generic message is supplied."""
    session_mock.post("DUMMY_ROUTE", status_code=denial_status_code, content="")
...
def test_generic_status_code_error(session_mock, flowclient_connection):
    ...
    session_mock.post(
        "DUMMY_ROUTE",
        status_code=418,
        content=ZMQReply(status="error", msg="I AM A TEAPOT"),
    )
...
def test_generic_status_code_unknown_error(session_mock, flowclient_connection):
...
def test_generic_status_code_no_payload(session_mock, flowclient_connection):
    """An error should be raised for status codes that aren't expected, with a blank payload if not given."""

    session_mock.post("DUMMY_ROUTE", status_code=418, content=dict(msg="DUMMY_ERROR"))
