...
async def test_query_ready_reports_true():
    ...
    con_mock = AsyncMock()
    con_mock.get_url = AsyncMock(return_value=AsyncMock(status_code=303))
    is_ready, reply = await query_is_ready(connection=con_mock, query_id="foo")
    assert is_ready
...
async def test_query_ready_raises():
    ...
    con_mock = AsyncMock()
    con_mock.get_url = AsyncMock(return_value=AsyncMock(status_code=999))
...
async def test_get_status_reports_finished():
    ...
    con_mock = AsyncMock()
    con_mock.get_url = AsyncMock(return_value=Mock(status_code=303))
    status = await get_status(connection=con_mock, query_id="foo")
...
async def test_get_status_404():
    ...
    con_mock = AsyncMock()
    con_mock.get_url = AsyncMock(side_effect=FileNotFoundError("DUMMY_404"))
...
async def test_get_status_raises():
    ...
    con_mock = AsyncMock()
    con_mock.get_url = AsyncMock(return_value=Mock(status_code=500))
    with pytest.raises(FlowclientConnectionError):
