...
#
RUN apt-get update \
            && apt-get install --yes --no-install-recommends build-essential postgresql-server-dev-$PG_MAJOR \
            && apt-get clean --yes \
            ...
            && apt-get remove -y postgresql-server-dev-$PG_MAJOR build-essential \
