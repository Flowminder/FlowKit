...
RUN cd /${SOURCE_TREE}/flowclient && python setup.py bdist_wheel && \
    cd /${SOURCE_TREE}/flowmachine/flowmachine-core && python setup.py bdist_wheel && \
    cd /${SOURCE_TREE}/flowmachine/flowmachine-queries && python setup.py bdist_wheel && \
    fix-permissions /${SOURCE_TREE}
USER $NB_UID
RUN pip install geopandas mapboxgl descartes \
    /${SOURCE_TREE}/flowclient/dist/*.whl \
    /${SOURCE_TREE}/flowmachine/flowmachine-core/dist/*.whl \
    /${SOURCE_TREE}/flowmachine/flowmachine-queries/dist/*.whl && \
